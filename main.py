...
def main():
    ...
    exp_name = str("[")+str(timestamp)+str("]")+str(grid_size)+str(connectivity[0])+str(obstacle_mode)+str(int(obstacle_density*100))+str(ranking_criteria)
    ...
    obstacles = obstacle_manager.place_obstacles(grid, config.SOURCE_NODES, config.DESTINATION_NODES)
    grid.add_obstacles(obstacles, config.SOURCE_NODES, config.DESTINATION_NODES)
    ...
    visualizer_forward.save(os.path.join(output_dir, "forward_paths_visualization.png"), config.SOURCE_NODES, config.DESTINATION_NODES)

    visualizer_backward = Visualizer(grid, obstacles, ranked_backward_paths)
    visualizer_backward.save(os.path.join(output_dir, "backward_paths_visualization.png"), config.SOURCE_NODES, config.DESTINATION_NODES)
