...
# Log file
LOG_FILE="$OUTPUT_DIR/parallel_run_log.txt"
: > $LOG_FILE  # Clear the log file

# Limit the number of parallel jobs to the number of available cores
MAX_JOBS=$(nproc)
job_count=0
    ...
    for obstacle_mode in "${obstacle_modes[@]}"; do
      for obstacle_density in "${obstacle_densities[@]}"; do
        for node_mode in "${node_modes[@]}"; do
          ...
          # Execute the experiment in the background
          echo "Running: $command" | tee -a $LOG_FILE
          eval $command >> $LOG_FILE 2>&1 &

          # Track the number of jobs and wait if necessary
          ((job_count++))
